...
{
    // Convert JLCcoin:// to JLCcoin:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("JLCcoin://"))
    {
        uri.replace(0, 12, "JLCcoin:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "JLCcoin.lnk";
}
...
{
    return GetAutostartDir() / "JLCcoin.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("JLCcoin-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  JLCcoin-qt [" + tr("command-line options") + "]                     " + "\n";
